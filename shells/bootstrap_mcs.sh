...
# This script compiles/installs MCS from scratch 
...
if [[ "$DISTRO" = 'bionic' || "$DISTRO" = 'focal' ]]; then
    ...
    MDB_CMAKE_FLAGS='-DWITH_SYSTEMD=yes -DPLUGIN_COLUMNSTORE=YES -DPLUGIN_MROONGA=NO -DPLUGIN_ROCKSDB=NO -DPLUGIN_TOKUDB=NO -DPLUGIN_CONNECT=NO -DPLUGIN_SPIDER=NO -DPLUGIN_OQGRAPH=NO -DPLUGIN_SPHINX=NO -DBUILD_CONFIG=mysql_release -DWITH_WSREP=OFF -DWITH_SSL=system -DDEB=bionic -DCMAKE_EXPORT_COMPILE_COMMANDS=1 -DWITH_EMBEDDED_SERVER=OFF'
